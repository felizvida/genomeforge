...
- Raw Sanger chromatogram visualization + sequencing verification/genotyping report (`/api/trace-chromatogram-svg`, `/api/trace-verify`)
- BLAST-like local nucleotide database search (`/api/blast-search`)
- Reference element libraries with auto-flag scan on loaded sequences (`/api/reference-db-save`, `/api/reference-db-list`, `/api/reference-db-load`, `/api/reference-scan`)
- siRNA design and binding-site mapping (`/api/sirna-design`, `/api/sirna-map`)
