...
UI_GUIDES = [
    ...
    {
        'match_api': '/api/compatibility-audit',
        'tab': 'Trace/Interop',
        'fields': ['Sequence / FASTA / GenBank', 'Topology', 'Compatibility Audit Formats'],
        'button': 'Audit Current Record',
        'panel': 'Compatibility Report',
        'sample': 'Use <code>genbank,sbol,genomeforge_dna,fasta</code> to see which formats preserve rich project metadata and which are sequence-only.',
        'troubleshoot': 'If a format needs review, inspect lost features, topology changes, qualifier loss, and CDS translation mismatches before trusting migration output.',
    },
    {
...
CASES = [
    ...
    case('AU', 'Compatibility Round-Trip Trust Audit', 'G', ['EGFP_CDS', 'pUC19_MCS'], 'Trace/Interop', 'Audit whether rich construct metadata survives export and re-import across GenBank, SBOL, portable DNA, and FASTA.', ['/api/compatibility-audit', '/api/compatibility-golden-project', '/api/convert-record'],
         'If you migrate a construct out of SnapGene or Geneious, how do you know which metadata survived instead of silently trusting the exported file?',
         'The case uses familiar reporter and vector elements because the metadata is the point: promoter, CDS, terminator, primer, qualifier, topology, and translation state all need to move with the sequence if the project is going to remain scientifically useful.',
         'Round-trip compatibility is a trust test. Exporting a file is not enough; you must re-import it and compare the biological meaning that came back. FASTA can be perfectly valid while still losing annotations. GenBank, SBOL, or a portable DNA container can be export-safe only if coordinates, qualifiers, topology, and CDS translation agree after the round trip.',
         'A migration report should feel like a lab notebook checklist: imported cleanly, warnings, lost metadata, export-safe, or needs review.',
         {'formats': ['genbank', 'sbol', 'genomeforge_dna', 'fasta'], 'golden_project_cases': 5, 'expected_safe_paths': ['GenBank', 'SBOL', 'portable DNA'], 'expected_review_path': 'FASTA metadata loss on annotated records'},
         ['A compatibility table with at least one export-safe path for every rich record.', 'A visible needs-review row for FASTA when annotations or topology matter.', 'A short migration decision saying which format you would use for handoff and why.'],
         ['Sequence-only success is not metadata success.', 'A native proprietary .dna import may be useful, but portable DNA export is not the same as writing SnapGene .dna.', 'Trust the format only after the re-imported biological meaning matches the source.'],
         'changing target formats or removing feature qualifiers',
         starter_values=[
             'Compatibility formats: <code>genbank,sbol,genomeforge_dna,fasta</code>',
             'Use <code>Audit Golden Project</code> to cover annotated plasmid, cloning design, expected edit, sequencing-confirmed construct, and multi-record bundle cases.',
             'Treat FASTA as sequence-only unless the report shows there was no metadata to preserve.',
         ]),
